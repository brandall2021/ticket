...
export default function AdminLinksPage() {
  ...
  const [editForm, setEditForm] = useState<Link | null>(null)

  const [showCreate, setShowCreate] = useState(false)
  const [createForm, setCreateForm] = useState({ title: "", description: "", url: "", imageUrl: "" })
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(false)
    setShowCreate(false)
    setCreateForm({ title: "", description: "", url: "", imageUrl: "" })
    fetchItems()
  }
    ...
    <div className="mx-auto max-w-7xl p-6">
      ...
      <Card>
        <CardContent className="p-0">
          {items.length === 0 ? (
            <p className="p-6 text-center text-sm text-neutral-500">Sin links</p>
          ) : (
